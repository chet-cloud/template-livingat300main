...
function set_posts_in_category(data,index,category_id){
    ...
    disable_next_pre_button(3, data.length,index)
    return [data.length,htm]
}

function disable_next_pre_button(page_size, total, index){
    setTimeout(() => {
        const pre_button = document.querySelector(".load-more-wrap").children[0]
        const next_button = document.querySelector(".load-more-wrap").children[1]
        if(index*3 >= total) { //disable next
            next_button.classList.add('no-more')
        }else{//able next
            next_button.classList.remove('no-more')
        }
        if(index <= 1){//disable pre
            pre_button.classList.add('no-more')
        }else{//able pre
            pre_button.classList.remove('no-more')
        }
    }, 1000);
}
