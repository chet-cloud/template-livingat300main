...
const search_callback = (html, search, callback) =>{
    const after = html.search(search)
    return callback(html.substring(0,after),html.substring(after))
}

const insert_in_head = (html, insert_file) =>{
    return search_callback(html, "</head>", (before,after)=>{
        return before + cat(insert_file) + after
    })
}
